function Y = model_G1_2_tip_deflection_mod_TE_dim_red(X)
...
* The model computes the tip deflection (trailing edge (TE)) as a function of uncertain variables (i.e., EI, GJ)
...
* Y: tip deflections corresponding to the different wing root angles (the angles arranged in increasing order of magnitude)
    * Y(1)      :static tip deflection (TE) at the smallest root angle (m)
    * Y(2)      :static tip deflection (TE) at the 2nd smallest root angle (m)
    ...
    * Y(5)      :static tip deflection (TE) at the largest root angle (m)
--------------------------------------------------------
%}
%%
...
[Y] = G1_2_tip_deflection_TE(run, angl, 'EI', X(1), 'GJ', 1);
[Y1] = G1_1_tip_deflection_TE(run, angl, 'EI', X(1), 'GJ', 1);
Y = Y-Y1(1);         % shift so that the deflection is zero at alpha=0
