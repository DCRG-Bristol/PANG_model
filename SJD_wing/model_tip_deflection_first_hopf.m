function Y = model_tip_deflection_first_hopf(X, P)
%% Title section - Wind-on tip deflections at first Hopf point versus AoA and E, G
%{
--------------------------------------------------------
Comments:
* The model computes the tip deflection at the first Hopf point as a function of uncertain variables (i.e., E, G)
...
for i=1:length(q_stat{1}(1,:))
    [x,y,z] = run.getDisplField(q_stat{1}(:,i), run.geom.L,...
        'beamModel');
    tipDisp_equib{1}(i) = z(1,1)/run.geom.L;  
end
...
if ~isempty(hopf_idx)
    tipDisp_1dim_HB{1} = tipDisp_equib{1}(hopf_idx);
else
    tipDisp_1dim_HB{1} = nan;
end

Y = tipDisp_1dim_HB{1}(1);
