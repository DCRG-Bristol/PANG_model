function Y = model_G2_frq_4_dim_red(X)
%% Title section - Fourth frequency versus wing root pitch angles and GJ, Sxx, Szz (G2 test)
%{
--------------------------------------------------------
Comments:
* This model refers to the G2 test (i.e., ground vibration test)
* The model computes the 4th modal frequency (torsion) as a function of uncertain variables (i.e., GJ, Sxx, Szz)
* The variable EI is fixed here (dimension reduction), as a result of sensitivity analysis
* The ratio between m_Sxx and m_Szz is preserved, hence only one uncertain parameter is considered (Sxx=Szz)
...
[Y] = G2_frq_4(run, angl, 'EI', 1, 'GJ', X(1), 'Sxx', X(2), 'Szz', X(2));
