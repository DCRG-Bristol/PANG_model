function Y = model_G2_frq_2(X)
%% Title section - Second frequency versus wing root pitch angles and EI, GJ, Sxx, Szz (G2 test)
%{
--------------------------------------------------------
Comments:
* This model refers to the G2 test (i.e., ground vibration test)
* The model computes the 2st modal frequency (IP bending) as a function of uncertain variables (i.e., EI, GJ, Sxx, Szz)
...
* Y: 2nd modal frequency corresponding to the different wing root angles (the angles arranged in increasing order of magnitude)
    * Y(1)      :2nd modal frequency at the smallest root angle (Hz)
    * Y(2)      :2nd modal frequency at the 2nd smallest root angle (Hz)
    ...
    * Y(6)      :2nd modal frequency at the largest root angle (Hz)
--------------------------------------------------------
%}
%%
...
[Y] = G2_frq_2(run, angl, 'EI', X(1), 'GJ', X(2), 'Sxx', X(3), 'Szz', X(4));
