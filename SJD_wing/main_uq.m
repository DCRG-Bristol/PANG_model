...
%% 7. Surrogates for flutter speed (first Hopf point) (uncertain variables: E, G; Uniform +-8%) 
% Description of the uncertain variables for UQLab
InputOpts.Marginals(1).Type = 'Uniform';
InputOpts.Marginals(1).Parameters = [0.92, 1.08]; % (uncertain scaling factor Young's modulus E) lower and upper uncertainty bound
InputOpts.Marginals(2).Type = 'Uniform';
InputOpts.Marginals(2).Parameters = [0.92, 1.08]; % (uncertain scaling factor modulus G for torsion) lower and upper uncertainty bound
myInput = uq_createInput(InputOpts); 
alp = ([0.3, 1.0, 0.6, 0.8])*pi/180; %angles of attack to run...
...
for ii = 3:length(alp)
    ...
    descriptive_title_for_plots = sprintf('%s surrogate (angle of attack(rad):%.2e)', MetaOpts.MetaType, alp(ii));
    N_eval = 100;                                                        % number of discretisation points for each uncertain variable (for plots)
    plotsfolderName = 'flutter_first_hopf_8_perc_E_G'; 
    subfolder_plotsfolderName = sprintf('aoa_case_%u', ii); % each 'case' refers to one fixed combination of design variables
    fullPath = fullfile(plotsfolderName, subfolder_plotsfolderName);
    mkdir(fullPath);
    mkdir(fullPath, 'plots_uq');
...
%% 8. Surrogates for root bending moment at the first Hopf point (uncertain variables: E, G; Uniform +-8%)
% Description of the uncertain variables for UQLab
InputOpts.Marginals(1).Type = 'Uniform';
InputOpts.Marginals(1).Parameters = [0.92, 1.08]; % (uncertain scaling factor Young's modulus E) lower and upper uncertainty bound
InputOpts.Marginals(2).Type = 'Uniform';
InputOpts.Marginals(2).Parameters = [0.92, 1.08]; % (uncertain scaling factor modulus G for torsion) lower and upper uncertainty bound
myInput = uq_createInput(InputOpts); 
alp = ([0.3, 1.0, 0.6, 0.8, 1.2])*pi/180; %angles of attack to run...
...
for ii = 5:length(alp)
    ...
    seed = 100;                     % seed for reproducibility due to randomness in sampling the training set
    N_train_increment = 8;          % we will increment the training set size until we reach convergence
    N_train_max = 650;              % training budget (i.e., maximum number of training points allowed)
    % run a test to check if surrogates are actually faster than classical MC for mean and sigma estimation  
    % recommended only for cheap models (to find the true mean and sigma, we need a large MC with the physical model) 
    flag_test_for_mean_and_sigma = false;
    flag_test_set = true;          % will a test set be generated for further surrogate validation?
    ...
    descriptive_title_for_plots = sprintf('%s surrogate (angle of attack(rad):%.2e)', MetaOpts.MetaType, alp(ii));
    N_eval = 100;                                                        % number of discretisation points for each uncertain variable (for plots)
    plotsfolderName = 'rbm_first_hopf_8_perc_E_G'; 
    subfolder_plotsfolderName = sprintf('aoa_case_%u', ii); % each 'case' refers to one fixed combination of design variables
    fullPath = fullfile(plotsfolderName, subfolder_plotsfolderName);
    mkdir(fullPath);
    mkdir(fullPath, 'plots_uq');
...
%% 9. Surrogates for tip deflection at the first Hopf point (uncertain variables: E, G; Uniform +-8%)
% Description of the uncertain variables for UQLab
InputOpts.Marginals(1).Type = 'Uniform';
InputOpts.Marginals(1).Parameters = [0.92, 1.08]; % (uncertain scaling factor Young's modulus E) lower and upper uncertainty bound
InputOpts.Marginals(2).Type = 'Uniform';
...
for ii = 5:length(alp)
    ...
    descriptive_title_for_plots = sprintf('%s surrogate (angle of attack(rad):%.2e)', MetaOpts.MetaType, alp(ii));
    N_eval = 100;                                                        % number of discretisation points for each uncertain variable (for plots)
    plotsfolderName = 'tip_deflection_first_hopf_8_perc_E_G'; 
    subfolder_plotsfolderName = sprintf('aoa_case_%u', ii); % each 'case' refers to one fixed combination of design variables
    fullPath = fullfile(plotsfolderName, subfolder_plotsfolderName);
    mkdir(fullPath);
    mkdir(fullPath, 'plots_uq');
...
%% 10. Discontinuous surrogates for root bending moment at the first Hopf point (uncertain variables: E, G) - AoA 1.2 deg

seed = 100;                     % seed for reproducibility
rng(seed, 'twister')

set(0, 'DefaultFigureVisible', 'off');
load('C:\Users\ua24606\git_2\PANG_model\SJD_wing\rbm_first_hopf_8_perc_E_G\aoa_case_5\training_set.mat');
low  = 0.92;   % example lower bound
high = 1.08;   % example upper bound
X_all = X(all(X >= low & X <= high, 2), :);
Y_all = Y(all(X >= low & X <= high, 2), :);
...
% Classification using deep learning
net_rbm = fitcnet(X_train, Y_train_lab,"LayerSizes",[40 40 40],'Activations','tanh');
[idl,clas] = net_rbm.predict(X_test); % Predict label at test set
...
for LOOP = 1:nclust
    ...
    PCEOpts.MetaType = 'PCE';
    PCEOpts.Degree = 1:15;
    myPCE_KS_rbm{LOOP} = uq_createModel(PCEOpts); % Save the PCE model
end

% Save the neural net and PCE into a file
save rbm_first_hopf_aoa_1_2_classification_and_local_models myPCE_KS_rbm net_rbm

% Predictions
Y_pred_hard = PCE_ensemble_hard_mixture_demo_rbm(X_test); % Hard mixture
...
plotsfolderName = 'rbm_first_hopf_8_perc_E_G'; 
subfolder_plotsfolderName = 'aoa_case_5_discontinuous';  % each 'case' refers to one fixed combination of design variables
fullPath = fullfile(plotsfolderName, subfolder_plotsfolderName);
mkdir(fullPath);
save(fullfile(fullPath, 'rbm_first_hopf_aoa_1_2_classification_and_local_models.mat'), 'myPCE_KS_rbm', 'net_rbm'); % save the surrogate
save(fullfile(fullPath, 'training_set.mat'), 'X_train', 'Y_train');             % Save both to a .mat file
save(fullfile(fullPath, 'validation_set.mat'), 'X_test', 'Y_test');             % Save both to a .mat file
save(fullfile(fullPath, 'validation_error.mat'), 'NMAE_hard', 'RMSE_hard');     % Save test performance to a .mat file   
...
% add readme to explain each 'case'
fileID = fopen(fullfile(fullPath, 'readme.txt'), 'w');
fprintf(fileID, 'PCE surrogates for the RBM at the first Hopf point as a function of the uncertain variables (the deterministic variables are fixed).\n\n');
fprintf(fileID, 'Legend:\n');
inputs_name = ["Young modulus (Pa)", "Torsion modulus (Pa)"];     % list of the names of the uncertain variables
N_variables = length(inputs_name);            % number of uncertain variables
outputs_name = ["Delta RBM (Nm) over EI"];    % list of the names of the QIs
...
%% 11. Discontinuous surrogates for tip deflection at the first Hopf point (uncertain variables: E, G) - AoA 1.2 deg

seed = 100;                     % seed for reproducibility
rng(seed, 'twister')

set(0, 'DefaultFigureVisible', 'off');
load('C:\Users\ua24606\git_2\PANG_model\SJD_wing\tip_deflection_first_hopf_8_perc_E_G\aoa_case_5\training_set.mat');
low  = 0.92;   % example lower bound
high = 1.08;   % example upper bound
X_all = X(all(X >= low & X <= high, 2), :);
Y_all = Y(all(X >= low & X <= high, 2), :);
...
% Classification using deep learning
net_tip_deflection = fitcnet(X_train, Y_train_lab,"LayerSizes",[40 40 40],'Activations','tanh');
[idl,clas] = net_tip_deflection.predict(X_test); % Predict label at test set
...
for LOOP = 1:nclust
    ...
    PCEOpts.MetaType = 'PCE';
    PCEOpts.Degree = 1:15;
    myPCE_KS_tip_deflection{LOOP} = uq_createModel(PCEOpts); % Save the PCE model
end

% Save the neural net and PCE into a file
save tip_deflection_first_hopf_aoa_1_2_classification_and_local_models myPCE_KS_tip_deflection net_tip_deflection

% Predictions
Y_pred_hard = PCE_ensemble_hard_mixture_demo_tip_deflection(X_test); % Hard mixture
...
fullPath = fullfile(plotsfolderName, subfolder_plotsfolderName);
mkdir(fullPath);
save(fullfile(fullPath, 'tip_deflection_first_hopf_aoa_1_2_classification_and_local_models.mat'), 'myPCE_KS_tip_deflection', 'net_tip_deflection'); % save the surrogate
save(fullfile(fullPath, 'training_set.mat'), 'X_train', 'Y_train');             % Save both to a .mat file
save(fullfile(fullPath, 'validation_set.mat'), 'X_test', 'Y_test');             % Save both to a .mat file
save(fullfile(fullPath, 'validation_error.mat'), 'NMAE_hard', 'RMSE_hard');     % Save test performance to a .mat file   
...
% add readme to explain each 'case'
fileID = fopen(fullfile(fullPath, 'readme.txt'), 'w');
fprintf(fileID, 'PCE surrogates for the tip deflection at the first Hopf point as a function of the uncertain variables (the deterministic variables are fixed).\n\n');
fprintf(fileID, 'Legend:\n');
inputs_name = ["Young modulus (Pa)", "Torsion modulus (Pa)"];     % list of the names of the uncertain variables
N_variables = length(inputs_name);            % number of uncertain variables
outputs_name = ["Tip deflection (m)"];        % list of the names of the QIs
