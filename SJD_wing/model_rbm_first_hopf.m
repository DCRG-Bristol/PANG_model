function Y = model_rbm_first_hopf(X,P)
%% Title section - Root bending moment at the first Hopf point versus AoA and E, G
%{
--------------------------------------------------------
Comments:
* The model computes the root bending moment at the flutter speed (i.e., the first Hopf point) as a function of uncertain variables (i.e., E, G)
...
* Y: vector of quantities of interest (QIs)
    * Y(1)      : (delta) root bending moment (Nm) divided by out-of-plane EI (to be compared with experimental measurements)
--------------------------------------------------------
%}
%%
...
EI1 = 2.268*X(1); %EI stored in this variable is used to compute My
...
dMy_equib{1} =...
    ...
    run.transF*q_stat{1}(run.structDisp,:) - My0;

if ~isempty(hopf_idx)
    dMy_1dim_HB{1} = dMy_equib{1}(hopf_idx);
else
    dMy_1dim_HB{1} = nan;
end

Y = dMy_1dim_HB{1}(1)/EI1; % a measure of strain (to be compared with experimental measurements)
end