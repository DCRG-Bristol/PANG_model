...
* experimental_data_set: is there experimental data available to verify the numerical predictions? (No - empty variable, Yes - read and plot)
--------------------------------------------------------
Output:
...
for ii = 1:N_variables
    ...
    for kk = 1:N_outputs
        ...
        % if N_variables == 1 & ~isempty(experimental_data_set)
        %     legend(sprintf('ExData (%.2e)', experimental_data_set(kk)), 'Location', 'best');    
        % end
