function uncertain_variables_exploration_for_discontinuities(input_distributions, inputs_names, outputs_names, plots_title, N_eval, seed, plotsfolderName, mean_Y, std_Y, X_train, Y_train)
...
N_outputs = length(outputs_names);                       % number of quantities of interest (QIs)
...
if N_variables >= 2
    ...
    for ii = 1:size(pairwise_combinations, 1)
        ...
        filteredMatrix = X_train(rowsToKeep, :);
...
corrMatrix_raw = corr(Y_train); % Compute correlation matrix
corrMatrix_spearman_raw = corr(Y_train, 'Type', 'Spearman'); % Compute Spearman correlation matrix
if N_outputs >= 2
    % Heatmap (pairwise correlations for the QIs)
    fig = figure();
    ...
    for ii = 1:size(pairwise_combinations, 1)
        fig = figure();
        scatter(Y_train(:, pairwise_combinations(ii, 1)), Y_train(:, pairwise_combinations(ii, 2)), 'filled', DisplayName='QIs');
        hold on;
        p = polyfit(Y_train(:, pairwise_combinations(ii, 1)), Y_train(:, pairwise_combinations(ii, 2)), 1); % Fit a linear regression line
        y_fit = polyval(p, Y_train(:, pairwise_combinations(ii, 1)));
        plot(Y_train(:, pairwise_combinations(ii, 1)), y_fit, 'r-', 'LineWidth', 2, DisplayName=sprintf('correlation: %.2f', corrMatrix_raw(pairwise_combinations(ii, 1), pairwise_combinations(ii, 2))));
        xlabel(outputs_names(pairwise_combinations(ii, 1)));
        ylabel(outputs_names(pairwise_combinations(ii, 2)));
        legend(Location='best')
