...
load('run_ONERA_a320.mat'); run = run_ONERA;

%% INITIALISATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

%ASSIGN BASELINE VALUES TO THE PARAMETERS
run = run.setPars('alpha0', 0*pi/180, 'g', 9.81, 'fuel_frac', 0.5, 'mach', 0.78);
...
% there is only 'fuel_frac', the fuel fraction.
...
for a_idx = 1:length(alp) %loop through pitch angles
    ...
    q = fsolve(@(qstr)(run.structDeriv(qstr,'alpha0', alp(a_idx))), q);

    [x, y, z] = run.getDisplField(q, xMesh(end) ,'beamModel'); %get displacements at x
    tipDefl_str(a_idx) = z(1,:); %z of size [2, length(xMeasureStat)] hold the z displacements in z(1,:) and TE in z(2,:)

    %eigenvalues....
    [~, e] = run.getStructModes(q, 'alpha0', alp(a_idx)); %this calculates the structural modes with the reference parameters...
    EigVals(:,a_idx) = e;
end

subplot(1,2,1); plot(alp*180/pi, tipDefl_str, 'x-', 'color', 'k'); hold on;
subplot(1,2,2);
for mode=1:4
    plot(alp*180/pi, abs(EigVals(mode,:))./(2*pi), 'x-', 'color', 'k'); hold on;
end
drawnow;
subplot(1,2,1);
...
Ur = linspace(5,175,15); %airspeed range
...
[T,Q] = ode45(@(t,x)(run.aero_structDeriv(x, 'U', U, 'alpha', pitch + (7.5*pi/180)*gustSeq_aoa(t))),...
    linspace(0,1,100), q);
