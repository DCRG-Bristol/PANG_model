function Y = model_evals_windoff_uncertain_pitch(X, P)
%% Title section - Modal frequencies versus pitch angle and E
%{
--------------------------------------------------------
Comments:
* The model computes the modal frequencies as a function of uncertain variables (i.e., pitch angle)
...
* P: deterministic parameters
    * P(1)      : Young's modulus E (Pa)
--------------------------------------------------------
Output:
* Y: vector of quantities of interest (QIs)
    * Y(1)      : modal frequency 1 (lowest)
    ...
    * Y(4)      : modal frequency 4
--------------------------------------------------------
%}
%%
...
q = fsolve(@(qstr)(run.structDeriv(qstr,'alpha0', X(1), 'E', P(1))), q); 

%eigenvalues....
[~, e] = run.getStructModes(q, 'E', P(1), 'alpha0', X(1)); %this calculates the structural modes with the reference parameters...
Y = abs(e(1:4))'./(2*pi);
