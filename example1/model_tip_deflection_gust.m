function Y = model_tip_deflection_gust(X, P)
%% Title section - 1MC gust max tip deflections versus airspeed and E, G
%{
--------------------------------------------------------
Comments:
...
* Y: vector of quantities of interest (QIs)
    * Y(1)      : max tip LE displacement in z direction (OOP) due to 1MC gust
--------------------------------------------------------
%}
%%
...
[T,Q] = ode45(@(t,x)(run.aero_structDeriv(x, 'U', P(1), 'alpha', pitch + (5*pi/180)*gustSeq_aoa(t))),...
    linspace(0,1,100), q);

for t_idx = 2:length(T)
    [x, y, z] = run.getDisplField(Q(t_idx,:)', xMesh(end), 'beamModel'); %get tip displacements at x
